...
async function createTables() {
    try {
        ...
        if (dividendRows[0].count === '0' || dividendRows[0].count === 0) {
            ...
            await sql`INSERT INTO dividend_data (id, data) VALUES (1, ${JSON.stringify(defaultDividendData)}) ON CONFLICT (id) DO NOTHING`;
        }

        // Seed pie_allocations
        const { rows: pieRows } = await sql`SELECT COUNT(*) as count FROM pie_allocations`;
        if (pieRows[0].count === '0' || pieRows[0].count === 0) {
            ...
            await sql`INSERT INTO pie_allocations (id, data) VALUES (1, ${JSON.stringify(defaultPieData)}) ON CONFLICT (id) DO NOTHING`;
        }

        // Seed yield_history
        const { rows: yieldRows } = await sql`SELECT COUNT(*) as count FROM yield_history`;
        if (yieldRows[0].count === '0' || yieldRows[0].count === 0) {
            ...
            await sql`INSERT INTO yield_history (id, data) VALUES (1, ${JSON.stringify(defaultYieldData)}) ON CONFLICT (id) DO NOTHING`;
        }
        ...
        if (portfolioRows[0].count === '0' || portfolioRows[0].count === 0) {
            const defaultPortfolioData = { portfolios: [], combinedGoal: 0 };
            await sql`INSERT INTO portfolio_data (id, data) VALUES (1, ${JSON.stringify(defaultPortfolioData)}) ON CONFLICT (id) DO NOTHING`;
        }
